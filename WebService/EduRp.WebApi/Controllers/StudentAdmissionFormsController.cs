...
namespace EduRp.WebApi.Controllers
{
    public class StudentAdmissionFormsController : ApiController
    {
        ...
        [HttpPut]
        public IHttpActionResult UpdateApplicationFormDocument([FromBody]List<GetApplicationFormDocumentDetail_Result> appFormDetail)
        {
            var isUpdate = stdadmfrmService.UpdateDocument(appFormDetail[0].UniversityId, appFormDetail);
            if (isUpdate == true)
                return Ok();
            return BadRequest();
        }
        ...
        [HttpPut]
        public IHttpActionResult UpdateApplicationFormHeader([FromBody]List<GetApplicationFormHeader_Result> appFormHeader)
        {
            var isUpdate = stdadmfrmService.UpdateHeader(appFormHeader[0].UniversityId, appFormHeader);
            if (isUpdate == true)
                return Ok();
            return BadRequest();
        }
